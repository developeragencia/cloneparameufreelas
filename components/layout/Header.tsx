...
export default function Header() {
  ...
  return (
    <header className="bg-[#2d3e50] text-white sticky top-0 z-50 shadow-lg">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-3">
            {session ? (
              <>
                <Link href="/dashboard/cliente/mensagens" className="text-gray-300 hover:text-white transition-colors p-2">
                  <MessageSquare className="w-5 h-5" />
                </Link>
                <Link href={getDashboardPath() + '/notificacoes'} className="text-gray-300 hover:text-white transition-colors p-2">
                ...
                <div className="relative">
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-white text-gray-800 rounded-lg shadow-xl border py-2 z-50">
                      ...
                      <hr className="my-1" />
                      <button onClick={() => signOut({ callbackUrl: '/' })} className="flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-gray-50 w-full text-left">
              ...
              <>
                <Link href="/login" className="text-white hover:text-gray-200 text-sm font-medium transition-colors px-3">
                  Login
                </Link>
                <Link href="/cadastro" className="text-white hover:text-gray-200 text-sm font-medium transition-colors px-3">
                  Cadastre-se
                </Link>
                <Link href="/publicar-projeto" className="bg-[#00aeef] hover:bg-[#0099d4] text-white text-sm font-semibold px-6 py-2.5 rounded transition-colors">
                  Publicar projeto
                </Link>
              </>
            )}
        ...
        {/* Secondary nav */}
        <div className="hidden md:flex items-center justify-end pb-3 gap-6 text-sm border-t border-gray-600 pt-2">
          <Link href="/como-funciona" className="text-gray-300 hover:text-white transition-colors">Como Funciona</Link>
          <Link href="/freelancers" className="text-gray-300 hover:text-white transition-colors">Encontrar Freelancers</Link>
          <Link href="/projetos" className="text-gray-300 hover:text-white transition-colors">Encontrar Trabalho</Link>
        </div>
      </div>

      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-[#1a252f] border-t border-gray-600 px-4 py-4 space-y-3">
          <form onSubmit={handleSearch} className="flex">
            ...
            />
            <button type="submit" className="bg-[#00aeef] px-4 py-2 rounded-r-md">
          ...
          <nav className="space-y-2">
            ...
            {session ? (
              <>
                ...
                <button onClick={() => signOut({ callbackUrl: '/' })} className="block text-red-400 hover:text-red-300 py-2 w-full text-left">Sair</button>
              </>
            ) : (
              <>
                ...
                <Link href="/publicar-projeto" className="block bg-[#00aeef] text-white text-center py-2.5 rounded">Publicar projeto</Link>
              </>
            )}
          </nav>
        </div>
