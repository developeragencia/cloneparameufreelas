  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Visibilidade do Perfil</h2>
        <div className="space-y-2">
          {[
            { value: 'public', label: 'Público', desc: 'Qualquer pessoa pode ver seu perfil' },
            { value: 'private', label: 'Privado', desc: 'Apenas clientes que você responde podem ver' },
          ].map(({ value, label, desc }) => (
            <label key={value} className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
              <input type="radio" name="visibility" value={value} checked={visibility === value} onChange={() => setVisibility(value)} className="mt-1 accent-[#00aeef]" />
              <div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="space-y-4">
          {[
            { key: 'email', label: 'Notificações por Email', desc: 'Receba novos projetos e mensagens por email' },
            { key: 'sms', label: 'Notificações por SMS', desc: 'Alertas urgentes por SMS' },
            { key: 'push', label: 'Notificações Push', desc: 'Notificações em tempo real no navegador' },
          ].map(({ key, label, desc }) => (
